...
**這是您要練熟的 6 個動作(2026-05-17 加發票編號防雙領):**
...
3. ⚠️ **輸入發票編號**(必填,小白單上「發票號碼」欄)
   - 例:`BM36258896` 或 `BM-36258896`(有沒有橫線都行,系統會自動處理)
   - 防雙領:**同一張發票編號全世界只能用一次**
   - 如果客人之前已經用過 → 系統會立刻顯示「⛔ 這張發票已被使用過」+ 詳細紀錄

4. **輸入消費金額**(例:5000)→ 點「✓ 產生 QR Code」
   - 系統會自動算擲骰機會數:**NT$2,000 = 1 次**,上限 5 次

5. **手機螢幕顯示 QR Code + 倒數 2 分鐘** → 給客人看

6. **客人用 LINE 相機掃這個 QR** → 客人 LIFF 直接收到擲骰機會
...
### E. 客訴查詢工具 `/admin/lookup`(2026-05-17 加)

**使用情境:** 客人來吵「我這張發票明明沒用過!」

1. **您手機開** `https://ickhh-culinary-game-v2.zeabur.app/admin/lookup`
2. **輸入 PIN**(同結帳 QR 的 PIN `91097496`,session 共用)
3. **輸入客人手上的發票編號**(從小白單抄,例:`BM36258896`)
4. **點查詢** → 系統顯示三種結果之一:

| 結果 | 畫面 | 怎麼處理 |
|------|------|---------|
| ✅ 此發票尚未使用 | 綠色 | 客人沒說謊,引導他自助掃發票或為他開結帳 QR |
| ⛔ 此發票已被使用 | 紅色(顯示**使用時間、使用者、桌號、金額、擲骰數**) | 把畫面給客人看,證據清楚 |
| ⚠️ 已產生 QR,客人尚未掃描 | 黃色 | 剛剛已開過 QR 沒掃,等過期或請客人立刻掃 |

**為什麼這個工具有用:**
- 系統有寫死「同一張發票編號只能用一次」,不是隨便擋人
- 紀錄不可竄改(寫進資料庫,連 IT 改都會留痕跡)
- 給客人看畫面 → 把問題從「你說我說」變成「系統紀錄這樣寫」
...
| `/admin/lookup` | 服務人員查發票紀錄(客訴用) | 數字 PIN | **2026-05-17 新增** |
