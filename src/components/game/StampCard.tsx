...
const REWARDS: Record<number, {name: string;shortName: string;isSpecial: boolean;tileImage: string;type: "lottery" | "fixed";}> = {
  ...
  6: { name: "NT$500 餐飲優惠券", shortName: "$500 餐飲券", isSpecial: true, tileImage: tileRewardMoney, type: "fixed" },
  7: { name: "機會/命運", shortName: "機會/命運", isSpecial: false, tileImage: tileLottery, type: "lottery" },
  8: { name: "NT$800 餐飲優惠券", shortName: "$800 餐飲券", isSpecial: true, tileImage: tileRewardMoney, type: "fixed" },
...
const StampCard = ({ totalPoints, maxPoints = 15, character }: StampCardProps) => {
  ...
  return (
    <div className="stamp-card overflow-visible p-0 rounded bg-cover bg-center bg-no-repeat" style={{ backgroundImage: `url(${stampCardBg})` }}>
      {/* Header */}
      <div className="items-center justify-between mb-3 px-4 pt-4 flex flex-col">
        <h3 className="text-lg font-bold text-foreground flex items-center gap-2 my-[15px]">洲遊味蕾旅遊地圖
